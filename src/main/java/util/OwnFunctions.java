...
public class OwnFunctions {
    ...
    public static double randomNumberFromTruncatedGaussian(double mean, double std, double min, double max) {
        ...
        while (result < min
            ...
            result = mean * (1 + std * randomNumber);//If this is modified, then please modify accordingly in threadRandomNumberFromTruncatedGaussian
        }
    ...
    public static double threadRandomNumberFromTruncatedGaussian(HasID object, double mean, double std, double min, double max) {
        ...
        while (result < min
            ...
            result = mean * (1 + std * randomNumber); //If this is modified, then please modify accordingly in randomNumberFromTruncatedGaussian
        }
    ...
    public static double average(ArrayList<Double> numbers, int firstIndex, int lastIndexExclusive) {
        if (lastIndexExclusive<=firstIndex || firstIndex>=numbers.size() || lastIndexExclusive > numbers.size()) return 0;
        double sum = 0;
    ...
    public static int firstNDecimalNumbersOfNonnegativeDouble(double doubleNumber, int nDecimalNumbers) {
    ...
    public static boolean csvHasHeaderLineWithNonNumericDataWithCommaSeparator(File file) {
        boolean hasHeaderLine = false;
        try {
            Scanner scanner = new Scanner(file);
            String firstLine = scanner.nextLine();
            String[] firstLineArray = firstLine.split(",");
            double firstValueAsDouble = Double.parseDouble(firstLineArray[0]);
            scanner.close();
