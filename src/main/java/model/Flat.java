...
public class Flat implements HasID {
    ...
    public double constructionCost = 0;
    public boolean newlyBuiltPricingUsed = false;
    ...
    ArrayList<ForSaleRecordTemporary> similarForSaleRecords = new ArrayList<>();
    ...
    //helper
    double marketPriceDataLoader;
    double originalMarketprice;
    ...
    public void calculateAndSetForSalePrice() {
        ...
        if (ownerConstructor != null)  {
            constructionCost = size * (bucket.neighbourhood.landPrice + getGeoLocation().constructionUnitCost);
            if (isZOP) constructionCost*=1+Model.ZOPConstructionCost;
            double constructionForSalePrice = constructionCost * (1 + Math.max(0.0,getGeoLocation().constructionMarkup));
            if (constructionForSalePrice>getMarketPrice()*Model.maxConstructionPriceToMarketPriceWithProperConstructionCostCoverage && getMarketPrice()/size > getGeoLocation().constructionUnitCost * Model.constructionCostCoverageNeed) constructionForSalePrice = getMarketPrice()*Model.maxConstructionPriceToMarketPriceWithProperConstructionCostCoverage;
            ...
            if (forSalePrice==getMarketPrice()) newlyBuiltPricingUsed=true;
        ...
        if (isForcedSale) {
            ...
            if (Model.realGDPLevelShockPath[Model.period]<=Model.realGDPLevelShockTriggerValueForAdditionalForcedSaleDiscount) forSalePrice *= (1-Model.additionalForcedSaleDiscount);
        }
    ...
    public double calculateMarketPriceBasedOnTwoClosestNeighbours() {
        ...
        for (FlatSaleRecord flatSaleRecord: bucket.getNeighbourhood().flatSaleRecordsForPrice) {
            ...
            if (sizeDistance>Model.sizeDistanceRatioThresholdForClosestNeighbourCalculation || stateDistance>Model.stateDistanceRatioThresholdForClosestNeighbourCalculation) continue;
    ...
    public double getMarketPrice() {
        ...
        if (marketPrice==0 && forSalePrice>0) return forSalePrice;
        return marketPrice;
    }
    ...
    public double calculateEstimatedMarketPrice() {
        if (Model.phase == Model.Phase.SETUP || Model.marketPriceOnRegression) return bucket.neighbourhood.priceRegressionFunctionLinear.flatPrice(this);
    ...
    public void incrementNBids(Household household) {
        synchronized(this) {
            nBids++;
        }
    }
    ...
    public boolean isEligibleForCSOK() {
        if (isNewlyBuilt && size>=Model.minSizeForCSOK) {
            return true;
        } else return false;
    }

    public boolean isEligibleForFalusiCSOK() {
        if (Model.period>=Model.introductionOfFalusiCSOK && Model.period<=Model.endOfFalusiCSOK && !isNewlyBuilt && getNeighbourhood().quality<Model.neighbourhoodQualityThresholdForFalusiCSOK && Model.phase!=Model.Phase.CAHAICALCULATIONS) {
            return true;
        } else return false;
    }
    ...
    public void refreshSimilarFlatSaleRecordsAndForSaleRecords() {
        ...
        for (ForSaleRecordTemporary forSaleRecord : bucket.getNeighbourhood().forSaleRecordsForPrice) {
